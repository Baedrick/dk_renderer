...
auto entry_point(dk::CmdLine *cmd_line) noexcept -> int {
	...
	for (Task *task = first_task; task != nullptr; task = task->next) {
		...
		for (PLT_DirIterResult file = {}; plt_dir_iter_next(arena, iter, &file); ) {
			String8 const file_path = str8f(arena, "%.*s/%.*s", DK_STR8_VARG(task->path), DK_STR8_VARG(file.name));
			if ((file.attributes.flags & PLT_FILE_FLAG_DIRECTORY) != 0) {
				Task *next_task = arena_push<Task>(arena);
